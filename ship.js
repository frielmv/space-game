...
const Ship = {
	...
	get engineFiring() {
		return Resources.fuel > 0 && (Controls.KEYS.w || (Viewport.clickTarget !== null && !Viewport.clickTarget.classList.contains('uiControl') && Viewport.clickTarget.id != 'ring'));
	},
	LANDING_MODE_ALT: 50, // how many meters above the surface the landing legs should extend
	landingMode: true,
	COLLIDERS: [
		[new Point(0.94, -42.75), false], // [point, is landing leg?]
		[new Point(-0.94, -42.75), false],
		[new Point(3.03, -41.805), false],
		[new Point(-3.03, -41.805), false],
		[new Point(5.209, -38.888), false],
		[new Point(-5.209, -38.888), false],
		[new Point(14.0, -23.25), false],
		[new Point(-14.0, -23.25), false],
		[new Point(24.507, 21.375), true],
		[new Point(-24.507, 21.375), true],
		[new Point(24.606, 41.415), true],
		[new Point(-24.606, 41.415), true]
	],
	ENGINE_GLOW_RAMP: 8, // time in seconds before engine glow hits max
	ACCEL: 20, // m/sec/sec
	CRASH_VEL: 20, // velocity the ship crashes at
	...
	init() {
		this.ANGLE_COLLIDERS = this.COLLIDERS.map(([point, leg]) => [point.angle, point.magnitude, leg]);
	...
	get alt() {
		return this.pos.magnitude - Bodies.soi.radius;
	},
	save() {
		return {
			posX: Ship.pos.x,
	...
	clock() {
		...
		if(Viewport.clickTarget !== null && !Viewport.clickTarget.classList.contains('uiControl')) {
