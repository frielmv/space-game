const Markers = {
	...
	init() {
		...
		this.mapAngle = (new html('img'))
			.class('marker')
			.attr({
				src: 'assets/markers/rotation-map.svg'
			})
			.style({
				pointerEvents: 'none',
				aspectRatio: '1 / 1',
	...
	clock() {
		...
		this.html.style({
			opacity: Markers.show && !Viewport.showMap ? '1' : '0'
		});
		...
		if(this.show) {
			...
			this.velCounter
				.style({
					...
					color: Ship.landingMode && Collisions.impactVel > Ship.CRASH_VEL ? 'var(--red)' : 'var(--ui)'
				})
